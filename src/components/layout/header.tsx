...
import { PasswordDialog } from "../shared/password-dialog";
...
export default function Header() {
  ...
  const navLinks = [
    ...
    { href: "/children", label: "Children", id: "children" as keyof SectionVisibility },
  ];

  const anySectionVisible = Object.values(sectionVisibility).some(v => v);
    ...
    <>
      <header className="sticky top-0 z-50 flex h-16 items-center justify-between gap-4 border-b bg-background/80 backdrop-blur-sm px-4 md:px-6">
        <div className="flex items-center gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem asChild>
                <Link href="/profile/user-1" className="flex items-center gap-2">
                  <User />
                  <span>Profile</span>
                </Link>
              </DropdownMenuItem>
          ...
          <Button variant="ghost" size="icon" className="rounded-full" onClick={() => setIsPasswordDialogOpen(true)}>
            <Lock className="h-5 w-5" />
            <span className="sr-only">Admin Lock</span>
          </Button>
        </div>
          ...
          <Sheet>
              ...
              <SheetContent side="right">
                <nav className="grid gap-6 text-lg font-medium">
                  ...
                  >
                    <Logo />
                    <span className="sr-only">YuvaSetu</span>
                  </Link>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                  <Link href="/" className="hidden md:flex items-center gap-2 font-semibold cursor-pointer">
                      <Logo />
                      <span className="text-3xl font-bold tracking-tighter font-headline">YuvaSetu</span>
                  </Link>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start">
                {navLinks.map(link => {
                  const isVisible = anySectionVisible ? sectionVisibility[link.id] : link.id === 'mainStream';
                  return isVisible && (
                    <DropdownMenuItem key={link.href} asChild>
                      <Link href={link.href} className="flex items-center justify-between">
                        <span>{link.label}</span>
                        {pathname === link.href && <Check className="h-4 w-4" />}
                      </Link>
                    </DropdownMenuItem>
                  )
                })}
