  ...
  useEffect(() => {
    async function fetchLiveVideos() {
      ...
      try {
        ...
        const activeLiveVideos = allVideos.filter(v => v.live && v.category === 'Farmer');
  ...
  return (
    <div className="space-y-8">
        <div className="text-center">
            ...
            <h1 className="text-4xl font-bold tracking-tight font-headline mt-4">Live Now: Farmer's Portal</h1>
            <p className="mt-2 text-lg text-muted-foreground">
                See what's broadcasting right now for farmers.
            </p>
        </div>
        <Card>
            <CardHeader>
                <CardTitle>Active Live Streams</CardTitle>
                <CardDescription>Live content specifically for the farming community.</CardDescription>
            </CardHeader>
            <CardContent>
                 {liveVideos.length > 0 ? (
                    ...
                    <p className="text-muted-foreground text-center py-8">
                        There are no active live streams for farmers at the moment.
                    </p>
                )}
            </CardContent>
        </Card>
