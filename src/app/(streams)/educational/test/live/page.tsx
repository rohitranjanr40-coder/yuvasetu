    ...
    useEffect(() => {
        if (isClient) {
            try {
                ...
                console.error("Failed to parse published tests from localStorage:", error);
            }
            setIsLoading(false); 
        }
    }, [isClient]);

    const moveToDrafts = (testToMove: PublishedTest) => {
        if (!isClient) return;
        try {
            // Remove from published tests
            const updatedPublishedTests = publishedTests.filter(test => test.id !== testToMove.id);
            localStorage.setItem('publishedTests', JSON.stringify(updatedPublishedTests));
            setPublishedTests(updatedPublishedTests);
            ...
            const newDraft = {
                ...
                // Ensure questions have a status if they don't already
                questions: testToMove.questions.map(q => ({...q, status: 'not-visited'}))
            };
            drafts.push(newDraft);
            localStorage.setItem('testDrafts', JSON.stringify(drafts));
            
            toast({
                title: 'Test Moved to Drafts',
                description: `"${testToMove.subject}" has been unpublished and moved back to drafts.`
            });
            ...
            toast({
                ...
                description: 'Could not move the test to drafts.'
            });
        }
    };
